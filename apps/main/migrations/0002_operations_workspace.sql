...
--> statement-breakpoint
-- ---------------------------------------------------------------------------
-- FK=OFF trigger mirror (same block as migrations-sqlite 0006; feishu-ops
-- 0002 precedent).
--
-- D1's FK enforcement default is disputed: Cloudflare docs state D1 enforces
-- foreign keys in all queries and migrations, while the repo's sql-client
-- adapter records the opposite empirical claim (better-sqlite3.ts:150-154 —
-- children-before-parents installs succeeded on D1). The mirror is the
-- dominant strategy under either regime — redundant when enforced,
-- load-bearing when not. Scope: the six operations tables only.
--
